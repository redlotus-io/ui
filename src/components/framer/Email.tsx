...
import { HiArchive, HiMail } from "react-icons/hi";
...
export const Email = () => {
  ...
  const toggleMessage = (mid: number) => {
    if (selectedMessages.includes(mid)) {
      setSelectedMessages(_messages => _messages.filter(id => id !== mid));
    } else {
      setSelectedMessages(_messages => [mid, ..._messages]);
    }
  };

  function addMessage() {
    const newId = (messages.at(-1) || 0) + 1;
    setMessages(_messages => [..._messages, newId]);
  }

  function archiveMessages() {
    setMessages(_messages => _messages.filter(id => !selectedMessages.includes(id)));
    setSelectedMessages([]);
  }
  ...
  return (
    <div className="flex h-screen flex-col items-center justify-center overscroll-y-contain bg-gradient-to-br from-slate-700 to-slate-900 py-8 px-6 text-slate-600">
      <div className="mx-auto flex w-full max-w-3xl flex-1 overflow-hidden rounded-2xl bg-white ">
        ...
        <div className="flex-1 overflow-y-scroll border-l p-8">
